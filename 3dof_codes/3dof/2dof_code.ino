...
void setup() {
  Serial.begin(9600); // Set to a higher baud rate for better communication
  Wire.begin();
  servo1.attach(18);
  servo2.attach(19);
  servo3.attach(17);
...
void loop() {
  ...
  if (mpuKalman()) {
    ...
    // Constrain the output to be within servo limits
    OutputX = constrain(OutputX, 0, 180);
    OutputY = constrain(OutputY, 0, 180);
    OutputZ = constrain(OutputZ, 0, 180);
  ...
  delay(10); // Small delay to reduce servo update frequency
}
